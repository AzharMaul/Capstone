...
''')
        ...
        if input_user == '1':
            tampilkan_daftar_peminjaman()  # Tampilkan semua peminjaman
        elif input_user == '2':
            cari_data_peminjaman()  # Cari peminjaman berdasarkan ID, nama, atau judul
        elif input_user == '3':
            print("Kembali ke Menu Utama.")
            break  # Kembali ke menu utama
        else:
...
def update_buku():
    while True:
        ...
        if pilihan == '1':
            ...
            if kriteria in daftar_buku['Id_buku']:
                index = daftar_buku['Id_buku'].index(kriteria)
            elif kriteria in daftar_buku['Judul']:
                index = daftar_buku['Judul'].index(kriteria)
            else:
            ...
            kolom = input("Masukkan nama kolom yang ingin diupdate (Judul/Author/Genre/Sub Genre/Status): ").capitalize()
            if kolom in daftar_buku:
...
def update_member():
    while True:
        ...
        if pilihan == '1':
            ...
            kolom = input("Masukkan nama field yang ingin diupdate (Nama/Nomor_hp/E-mail/Alamat): ").capitalize()
            if kolom in daftar_member:
                new_value = input(f"Masukkan nilai baru untuk {kolom}: ")
...
def hapus_buku():
    while True:
        ...
        if pilihan == "1":
            ...
            if input_buku in daftar_buku['Id_buku']:
                index = daftar_buku['Id_buku'].index(input_buku)
                for key in daftar_buku.keys():
                    daftar_buku[key].pop(index)
                print(f"Buku dengan ID '{input_buku}' berhasil dihapus.")
            elif input_buku in daftar_buku['Judul']:
                index = daftar_buku['Judul'].index(input_buku)
                for key in daftar_buku.keys():
                    daftar_buku[key].pop(index)
                print(f"Buku dengan Judul '{input_buku}' berhasil dihapus.")
            else:
                print(f"Buku dengan ID atau Judul '{input_buku}' tidak ditemukan.")
        
        elif pilihan == "2":
            print("Kembali ke Main Menu...")
            main_menu()  # Memanggil kembali fungsi main_menu
            break
        else:
...
def pengembalian_buku():
    while True:
        ...
        if pilihan == "1":
            ...
            if input_pengembalian in daftar_peminjaman['Id_Pinjam']:
                index = daftar_peminjaman['Id_Pinjam'].index(input_pengembalian)
            ...
            elif input_pengembalian in daftar_peminjaman['Judul_buku']:
                index = daftar_peminjaman['Judul_buku'].index(input_pengembalian)
                for key in daftar_peminjaman.keys():
                    daftar_peminjaman[key].pop(index)
                print(f"Buku dengan Judul '{input_pengembalian}' berhasil dikembalikan.")
            elif input_pengembalian in daftar_peminjaman['Nama_Peminjam']:
                index = daftar_peminjaman['Nama_Peminjam'].index(input_pengembalian)
                for key in daftar_peminjaman.keys():
                    daftar_peminjaman[key].pop(index)
                print(f"Buku yang dipinjam oleh '{input_pengembalian}' berhasil dikembalikan.")
            else:
...
def menu_admin():
    ...
    while True:
        print('''
Main Menu : 1. Tampilkan Daftar Buku
            2. Cari Buku
            3. Tambah Buku
            4. Update Buku
            5. Hapus Buku  
...
''')
        ...
        if pilihan == '1' :
            ...
            tampilkan_daftar_peminjaman()
            back = input('Tekan \"Enter\" untuk kembali ke Main Menu : ')
            continue
        elif pilihan == '8' :
            perpanjang_pinjaman()
            continue
...
def menu_member():
    ...
    while True:
        print('''
Main Menu : 1. Daftar Buku
            2. Cari Buku
            3. Pinjam Buku
            4. Tampilkan Daftar Peminjaman
            5. Perpanjang Peminjaman Buku
            6. Pengembalian Buku
            7. Daftar Member 
            8. Tampilkan Daftar Member
            9. Exit Program
''')
